...
#[derive(Debug)]
pub struct TaskCache(HashSet<CachedTask>);

impl<'a> Default for TaskCache {
    fn default() -> Self {
        let mut set = HashSet::with_capacity(10);
        let list = vec![
            Task::default().summary("Task A.1"),
            Task::default().summary("Task B.2"),
            Task::default().summary("Task C.3"),
        ];
...
impl TaskList for TaskCache {
    ...
    fn remove<T: Into<Uuid>>(&mut self, id: T) -> bool {
        let id = id.into();
        self.0.remove(&CachedTask(Task::with_id(id)))
    }
}
